...
export async function getLeaderboard() {
  try {
    ...
    const { data, error } = await supabase
      .from('leaderboard')
      .select('user_name, points, badge_count')
...
export async function updateLeaderboard(user) {
  try {
    ...
    // 1. Check if user already exists to compare points
    ...
    // 2. If user exists and new score is not strictly greater, skip update.
    if (existingUser && user.points <= Number(existingUser.points)) {
      return; // Do nothing if the new score isn't an improvement
    }

...
export function checkMilestones(counts) {
  const achieved = [];
  let next = null;
  const totalGamesForMilestone = counts.game + (counts.specialGame || 0) + (counts.specialGame3 || 0);

  // Assuming milestones are sorted in ascending order of requirements in config
  for (const milestone of milestones) {
    if (totalGamesForMilestone >= milestone.gamesRequired && counts.skillBadge >= milestone.skillBadgesRequired) {
      achieved.push(milestone);
    } else {
      next = {
        ...milestone,
        gamesNeeded: Math.max(0, milestone.gamesRequired - totalGamesForMilestone),
 ...
 * Cutoff: July 13, 2026 at 12:00 AM UTC (UTC: 2026-07-13T00:00:00Z)
 */
export function checkFacilitatorMilestones(badges) {
  const CUTOFF_TIMESTAMP = Date.parse("2026-07-13T00:00:00Z");

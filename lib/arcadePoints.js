...
export function classifyBadge(badgeName) {
  ...
  if (knownSkillBadges.has(normalizedName) || knownSkillBadges.has(lowerName)) {
    return 'skillBadge';
  }
...
export function calculateScore(badges) {
  ...
  const totalPoints = points.game + points.special + points.specialGame + points.specialGame3 + points.skillBadge;
